...
router.post('/kyc/:id', async (req, res) => {
    try {
        ...
        const driver = await User.findByIdAndUpdate(req.params.id, { kycStatus: status }, { new: true });
...
router.patch('/drivers/:id/status', async (req, res) => {
    try {
        ...
        const driver = await User.findByIdAndUpdate(req.params.id, { isSuspended }, { new: true });
        res.json({ success: true, driver });
    }
    catch (error) {
        res.status(500).json({ success: false, error: error.message });
...
router.post('/config', async (req, res) => {
    try {
        ...
        const config = await Config.findOneAndUpdate({ key }, { value, description, updatedAt: new Date() }, { upsert: true, new: true });
        res.json({ success: true, config });
    }
    catch (error) {
        res.status(500).json({ success: false, error: error.message });
...
router.patch('/reports/:id', async (req, res) => {
    try {
        ...
        const report = await Report.findByIdAndUpdate(req.params.id, { status, resolvedAt: status === 'resolved' ? new Date() : undefined }, { new: true });
